...
async def tempRoleCheck():
    database = sqlite3.connect("bot.db")
    c = database.cursor()
    temproles = c.execute("SELECT * FROM temprole")
    temproles_f = temproles.fetchall()
    for entry in temproles_f:
        if int(entry[2]) <= round(time.time()):
            roletoremove = bot.get_guild(1138508679608533122).get_role(int(entry[3]))
            userfromremove = bot.get_guild(1138508679608533122).get_member(int(entry[0]))
            await userfromremove.remove_roles(roletoremove, reason="Temprole expired")
            c.execute(f"DELETE FROM temprole WHERE uid = \"{entry[0]}\" AND roleid = \"{entry[3]}\"")
            print(f"Removed {roletoremove} from {userfromremove} because time is {time.time()} and temproles expire is {entry[2]}")
            database.commit()
    c.close()
    database.close()
...
@bot.event
async def on_ready():
    maintenancerm = bot.get_channel(1208826268246876312)
    await maintenancerm.send("`Initalizing Database...`")
    await dbinit()
    await maintenancerm.send("`Setting up variables...`")
    memberschannel = bot.get_channel(1201442228011270174)
    cmdchannel = bot.get_channel(1208826268246876312)
    now = datetime.now()
    global current_time
    current_time = now.strftime("%D %H:%M:%S")
    print(f"Bot started with {Fore.LIGHTBLUE_EX}{bot.user}{Fore.RESET} ID |{Fore.LIGHTMAGENTA_EX} {current_time} {Fore.RESET}")
    await bot.change_presence(activity=discord.Activity(name = f"/info | Az eredeti Jett RP bot!", type=discord.ActivityType.watching))
    await maintenancerm.send("`Setting up activity`")
#    added_users = []
...
#            await cmdchannel.send(f"Couldnt add roles to {bot.get_guild(1075140354656981053).members[i]}, error: {err}")
    await maintenancerm.send("`Syncing commands...`")
    try:
        await bot.tree.sync()
    except Exception as e:
        await cmdchannel.send("Commands failed to sync: {e}")
    await maintenancerm.send("`Initalizing buttons...`")
    bot.add_view(Help())
    await maintenancerm.send("`Starting loop coroutines...`")
    tempRoleCheck.start()
    await maintenancerm.send("`Bot started!`\n- Bot HEARTBEAT: `" + str(round(bot.latency * 1000)) + " ms`")

@bot.event
async def on_connect():
    print("Successfully connected to discord")
    maintenancerm = bot.get_channel(1208826268246876312)
    await maintenancerm.send("Successfully connected to discord")

@bot.event
async def on_disconnect():
    print("Disconnected from discord, restarting...")
    await startBot()
    maintenancerm = bot.get_channel(1208826268246876312)
    await maintenancerm.send("Reconnected to discord! \nCurrent bot HEARTBEAT: `" + str(round(bot.latency * 1000)) + "`")

@bot.event
async def on_shard_connect(shard_id):
    print("Connected Shard ID to discord")
    maintenancerm = bot.get_channel(1208826268246876312)
    await maintenancerm.send("Connected Shard ID to discord (`" + str(shard_id) + "`)")

@bot.event
async def on_shard_disconnect():
    print("Shard ID disconnected from discord, reconnecting...")
    try:
        print("Reconnecting...")
        await discord.ShardInfo.reconnect()
    except:
        try:
            print("Reconnection failed, connecting manually...")
            await discord.ShardInfo.connect()
        except:
            print("Connection failed, disconnecting and retrying...")
            await discord.ShardInfo.disconnect()
            await asyncio.sleep(3)
            await discord.ShardInfo.connect()
    print("Successfully connected Shard ID to discord")
    maintenancerm = bot.get_channel(1208826268246876312)
    await maintenancerm.send("Reconnected Shard ID to discord!\n Current shard HEARTBEAT: `" + str(discord.ShardInfo.latency) + "`")
...
async def on_voice_state_update(member, before, after):
    ...
    if before.channel is None and after.channel is not None:
        privatevoice = discord.utils.get(member.guild.voice_channels, id=1208826832216920085)
...
async def on_member_join(user):
    ...
    cmdchannel = bot.get_channel(1201442228011270174)
    await cmdchannel.send(f"{user} joined the server, total users {user.guild.member_count}")
    added = user.guild.get_role(1208827122001518633)
...
async def info(interaction: discord.Interaction):
    ...
    if not interaction.channel.id == 1107018544236998787: 
        ...
        embed.add_field(name="Owners", value=f"- Oreo\n- Kiskarbu", inline=False)
        embed.add_field(name="Fivem IP(v4) cím", value=f"> `87.229.6.72`", inline=True)
...
async def nitro(interaction: discord.Interaction, amount: int):
    ...
    if not interaction.channel.id == 1107018544236998787:
        premiumrole = interaction.user.guild.get_role(1208827569617510460)
    ...
    if not interaction.channel.id == 1107018544236998787:
        ...
        if queryresultfetched is None:
            ...
            if itemsfound == 0:
                ...
                await interaction.response.send_message(embed = embed, view=Sell(moneyget=moneyget, itemsfound=itemsfound, inv=inv) ,ephemeral=True)

class Sell(discord.ui.View):
    def __init__(self, moneyget : int, itemsfound : int, inv : list | dict):
        super().__init__()
        self.value = None
        try: 
            self.moneyget : int = moneyget
            self.itemsfound : int = itemsfound
            self.inv : list | dict = inv
        except:
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if queryresultfetched is None:
            ...
            c.execute(f"UPDATE money SET cash = {queryresultfetched[0] + self.moneyget} WHERE uid = \"{interaction.user.id}\"")
        database.commit()
        c.close()
        database.close()
        embed = discord.Embed(title="Sell", description=f"You sold **{self.itemsfound}** items for `{self.moneyget}$`", color=discord.Color.blue())
        await interaction.response.send_message(embed = embed)
        self.value = False
        self.moneyget, self.itemsfound, self.inv = None, None, None
        self.stop()
...
#async def tcp_attack(interaction: discord.Interaction, ip: str, port: str, packets: int, size: int):
#    if packets <= 25000:
#        if size <= 61440:
#            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
#            bytes = random._urandom(int(size))
#            sent = 0
#            embed = discord.Embed(title="TCP attack", description=f"TCP attack in progress at {ip}:{port}", color=discord.Color.blue())
#            await interaction.response.send_message(embed = embed)
#            async def startddos(sent):
#                while True:
#                    sockresult = sock.sendto(bytes,(ip, int(port)))
#                    sent += 1
#                    if sent >= 25000:
#                        print("DDoS attack finished")
#                        embed = discord.Embed(title="TCP attack", description=f"TCP attack finished", color=discord.Color.blue())
#                        await interaction.channel.send(embed = embed)
#                        break
#                    print("Sent " + str(sent) + " packets to " + ip + ", answer: "+ str(sockresult))
#            startddos(sent)
#        else:
#            await interaction.response.send("Too big packets to send. Max 61440 is allowed")
#    else:
#        await interaction.response.send("Too much packets to send. Max 25000 allowed.")
...
async def mine(interaction: discord.Interaction):
    ...
    if not interaction.channel.id == 1107018544236998787:
        ...
        if interaction.user.id in minecl and time.time() - minecl[interaction.user.id] < 30:
            ...
            database = sqlite3.connect("bot.db")
            c = database.cursor()
            select = c.execute(f"SELECT * FROM inventory WHERE uid = \"{interaction.user.id}\" AND itemname = \"Pickaxe\"")
            select_f = select.fetchone()
            if select_f is None:
                ...
                if chance > 2:
                    ...
                    for x in range(amount):
                        database = sqlite3.connect("bot.db")
                        c = database.cursor()
                        checkifexists = c.execute(f"SELECT * FROM inventory WHERE uid = \"{interaction.user.id}\" AND itemname = \"{item}\"")
                        checkifexists_f = checkifexists.fetchone()
                        if checkifexists_f is None:
                            print("new")
                            c.execute(f"INSERT INTO inventory (uid, username, itemname, amount) VALUES (\"{interaction.user.id}\", \"{interaction.user}\", \"{item}\", 1)")
                        else:
                            print("exists")
                            currentamount = checkifexists_f[4]
                            c.execute(f"UPDATE inventory SET amount = {currentamount + 1} WHERE uid = \"{interaction.user.id}\" AND itemname = \"{item}\"")
                        database.commit()
                        c.close()
                        database.close()
                    embed = discord.Embed(title="Mine", description=f"You successfully found and mined `{amount}x` `{item}`!", color=discord.Color.blue())
                    await interaction.response.send_message(embed = embed)
                else:
                    embed = discord.Embed(title="Mine", description=f"Even a pickaxe couldn't help on your problem, you still found nothing... Maybe get a flashlight or idk.", color=discord.Color.blue())
                    await interaction.response.send_message(embed = embed)
...
async def dig(interaction: discord.Interaction):
    ...
    if not interaction.channel.id == 1107018544236998787:
        ...
        if interaction.user.id in digcl and time.time() - digcl[interaction.user.id] < 30:
            ...
            database = sqlite3.connect("bot.db")
            c = database.cursor()
            select = c.execute(f"SELECT * FROM inventory WHERE uid = \"{interaction.user.id}\" AND itemname = \"Shovel\"")
            select_f = select.fetchone()
            if select_f is None:
                ...
                if random.randint(0, 6) > 1:
                    ...
                    for x in range(amount):
                        database = sqlite3.connect("bot.db")
                        c = database.cursor()
                        checkifexists = c.execute(f"SELECT amount FROM inventory WHERE uid = \"{interaction.user.id}\" AND itemname = \"{item}\"")
                        checkifexists_f = checkifexists.fetchone()
                        if checkifexists_f is not None:
                            print("exists")
                            itemsamount = checkifexists_f[0]
                            c.execute(f"UPDATE inventory SET amount = {itemsamount+1} WHERE uid = \"{interaction.user.id}\" AND itemname = \"{item}\"")
                            database.commit()
                            c.close()
                            database.close()
                        else:
...
async def use(interaction: discord.Interaction, item: discord.app_commands.Choice[int], user: typing.Optional[discord.Member]):
    ...
    if not interaction.channel.id == 1107018544236998787:
        ...
        if item.value == 1:
            database = sqlite3.connect("bot.db")
            c = database.cursor()
            select = c.execute(f"SELECT * FROM inventory WHERE uid = \"{interaction.user.id}\" AND itemname = \"{item.name}\"")
            select_f = select.fetchone()
            if select_f is not None:
                #tresitem = random.choice(shopitems)
                randomindex = random.randint(0, (shopitems.__len__() - 1))
                print(randomindex)
                tresitem = ""
                for ditem, dvalue in enumerate(shopitems.items()):
                    if ditem == randomindex:
                        print("ditem: ", ditem)
                        print("dvalue: ", dvalue)
                        print("randomindex: ",  randomindex)
                        tresitem = dvalue[0]
                checkifexists = c.execute(f"SELECT * FROM inventory WHERE uid = \"{interaction.user.id}\" AND itemname = \"{tresitem}\"")
                checkifexists_f = checkifexists.fetchone()
                if checkifexists_f is not None:
                    c.execute(f"UPDATE inventory SET amount = {checkifexists_f[4] + 1} WHERE uid = \"{interaction.user.id}\" AND itemname = \"{tresitem}\"")
                else:
                    c.execute(f"INSERT INTO inventory (uid, username, itemname, amount) VALUES (\"{interaction.user.id}\", \"{interaction.user}\", \"{tresitem}\", 1)")
                if select_f[4] > 1:
                    c.execute(f"UPDATE inventory SET amount = {select_f[4] - 1} WHERE uid = \"{interaction.user.id}\" AND itemname = \"{item.name}\"")
                else:
                    c.execute(f"DELETE FROM inventory WHERE uid = \"{interaction.user.id}\" AND itemname = \"{item.name}\"")
                database.commit()
                c.close()
                database.close()
...
async def fish(interaction: discord.Interaction):
    ...
    if not interaction.channel.id == 1107018544236998787:
        if interaction.user.id in economybanneds:
            ...
            await interaction.response.send_message(embed = embed)
            return
        if interaction.user.id in fishcl and time.time() - fishcl[interaction.user.id] < COOLDOWN:
            embed = discord.Embed(title="Trash", description=random.choice(cooldwnsays), color=discord.Color.blue())
            await interaction.response.send_message(embed = embed)
        else:
            database = sqlite3.connect("bot.db")
            ...
            c.close()
            database.close()
        fishcl[interaction.user.id] = time.time()
...
async def report(interaction: discord.Interaction, user: discord.Member, type: discord.app_commands.Choice[int], reason: str):
    ...
    if not interaction.channel.id == 1107018544236998787:
        ...
        embed = discord.Embed(title="Report", description=f"You sent a report to the admin team. The reported person: {user.mention} | The report type: `{str(type.name)}` | The reason: `{reason}`.", color=discord.Color.blue())
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    if not interaction.channel.id == 1107018544236998787:
        ...
        if interaction.user.guild_permissions.mute_members:
            if interaction.user.top_role > user.top_role:
                await user.add_roles(user.guild.get_role(1208828103653064754))
                ...
                await user.remove_roles(user.guild.get_role(1208828103653064754))
    ...
    if interaction.user.guild_permissions.manage_messages:
        ...
        if queryresultfetched is None:
            ...
            if (queryresultfetched[0] + 1) == 5:
                modchannel = bot.get_channel(1208826268246876312)
...
class WarnAction(discord.ui.View):
    ...
    async def warnsoftmute(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.user.add_roles(self.user.guild.get_role(1208828103653064754))
        ...
        await self.user.remove_roles(self.user.guild.get_role(1208828103653064754))
...
async def rickroll(interaction: discord.Interaction, user: discord.Member):
    ...
    try:
        vchannel = await interaction.guild.fetch_channel(1198377725375823992)
        vclient = await vchannel.connect()
        await user.send(f"hello, get in {vchannel.mention}\n pls i wanna show u smthng :3 ")
        await interaction.channel.send("Okay, i prepared everything. Im waiting...")
        ...
        global vid
        vid = path + filen
        vclient.play(discord.FFmpegPCMAudio(executable=".//ffmpeg", source=vid))
...
class MusicDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            vclient.play(discord.FFmpegPCMAudio(executable=".//ffmpeg", source=path+filen))
        except Exception as ex:
            print(ex)
            for vclient in bot.voice_clients:
    ...
    if interaction.user.voice is not None:
        try:
            ...
            try:
                ...
                print("Playing video...")
                player = voiceclient.play(discord.FFmpegPCMAudio(executable=".//ffmpeg", source=vid))
            except:
                print("Playing video...")
                for vclient in bot.voice_clients:
                    if vclient.channel == interaction.user.voice.channel:
                        player = vclient.play(discord.FFmpegPCMAudio(executable=".//ffmpeg", source=vid))
        except Exception as error:
...
async def start_loop(interaction: discord.Interaction):
    try:
            voiceclient.stop()
            await asyncio.sleep(1)
            player = voiceclient.play(discord.FFmpegPCMAudio(executable=".//ffmpeg", source=vid), after=lambda e: asyncio.run_coroutine_threadsafe(loop_check(interaction), bot.loop).result())
    except:
...
@bot.tree.command(name="temprole")
async def temprole(interaction: discord.Interaction, user: discord.Member, role: discord.Role, expires: str):
    """Add a role to a user temporarily
    
    Parameters
    -----------
    expires: str
        Enter it in hours, for example: '3h' or '1.5 h'"""
            ...
            if "h" in expires:
                for i in range(len(expires)):
                    try:
                        ...
                        continue
                if expires_int == 0:
                    embed = discord.Embed(title="Temprole", description="Invalid format", color=discord.Color.blue())
                    await interaction.response.send_message(embed=embed)
                    return 0
            else:
                embed = discord.Embed(title="Temprole", description="Invalid format", color=discord.Color.blue())
                await interaction.response.send_message(embed=embed)
                return 0
            c.execute(f"INSERT INTO temprole (uid, username, expires, roleid) VALUES (\"{user.id}\", \"{user}\", \"{round(time.time()+((expires_int*60)*60))}\", \"{role.id}\")")
            await user.add_roles(role, reason="Temprole add")
            database.commit()
            c.close()
            database.close()
            embed = discord.Embed(title="Temprole", description=f"Successfully added {role} to {user} for {expires}", color=discord.Color.blue())
            await interaction.response.send_message(embed=embed)
        else:
            embed = discord.Embed(title="Temprole", description="This user already has this role.", color=discord.Color.blue())
            await interaction.response.send_message(embed=embed)

@bot.tree.command(name="temproles")
...
async def restart(interaction: discord.Interaction):
    ...
    for vclient in bot.voice_clients:
        if vclient.channel == interaction.user.voice.channel:
            vclient.stop()
            await asyncio.sleep(1)
            await vclient.play(discord.FFmpegPCMAudio(executable=".//ffmpeg", source=vid))
...
async def ezforence(interaction: discord.Interaction):
    ...
    try:
        vclient = await interaction.user.voice.channel.connect()
        vclient.play(discord.FFmpegPCMAudio(executable=".//ffmpeg", source="easter_egg_1.mp3"))
    except:
        for vclient in bot.voice_clients:
            if vclient.channel == interaction.user.voice.channel:
                vclient.play(discord.FFmpegPCMAudio(executable=".//ffmpeg", source="easter_egg_1.mp3"))
    ...
    for vclient in bot.voice_clients:
        if vclient.channel == interaction.user.voice.channel:
            ...
            for i in range(len(queue)):
                if "./tmp/"+queue[i] == currentsong:
                    print("geci")
                    if i != len(queue):
                        print("gec")
                        vclient.stop()
                        print("queue: ./tmp/"+queue[i+1])
                        print("currentsong: " + currentsong)
                        vclient.play(discord.FFmpegPCMAudio(executable=".//ffmpeg", source="./tmp/"+queue[i+1]))
                        embed = discord.Embed(title="Next", description=f"Skipping to the next music", color=discord.Color.blue())
                        nowplaying = "./tmp/"+queue[i+1]
                        global skipped
                        skipped = "./tmp/"+queue[i+2]
                        await interaction.response.send_message(embed=embed)
                    else:
                        embed = discord.Embed(title="Next", description=f"The bot is not playing anything", color=discord.Color.blue())
                        await interaction.response.send_message(embed=embed)
    if not found:
        embed = discord.Embed(title="Next", description=f"The bot is not in your voice channel", color=discord.Color.blue())
        await interaction.response.send_message(embed=embed)
    ...
    if operation:
        if operation.value == 1:
            ...
            for current in songs:
                ...
                global skipped
                global stopped
                if skipped and isinstance(skipped, str):
                    print("skipped: ", skipped.split("./tmp/")[1])
                    print("current: ", current)
                    if skipped.split("./tmp/")[1] != current:
                        continue
                print("playing: " + path + filen)
                try:
                    vclient = await interaction.user.voice.channel.connect()
                    vclient.play(discord.FFmpegPCMAudio(executable=".//ffmpeg", source=path+filen))
                    await interaction.channel.send(f"Playing {current} for {interaction.user.voice.channel.mention}")
                    nowplaying = path+filen
                    while vclient.is_playing():
                        await asyncio.sleep(1)
                        if vclient.is_paused():
                            while vclient.is_paused():
                                await asyncio.sleep(1)
                        if stopped:
                            print("Stopped")
                            stopped = False
...
async def on_message(message):
    ...
    if message.channel.id == 1208830795440193626:
        if message.author in codes and mscontent[0] == codes[message.author]:
            ...
            verifyrem = discord.utils.get(message.guild.roles, id=1208827122001518633)
            await message.author.remove_roles(verifyrem)
            verifyadd = discord.utils.get(message.guild.roles, id=1198383575553286154)
    ...
    try:
        ...
        if does_exist is None:
            ...
            if((newxp//100) > (currentxp // 100)):
                levelchannel = message.guild.get_channel(1212045521435492422)
            ...
            if newxp//100 >= 5:
                if message.guild.get_role(1175739448940564532) not in message.author.roles:
                    await message.author.add_roles(message.guild.get_role(1212044967699284039))
                    levelchannel = message.guild.get_channel(1212045521435492422)
    ...
    if "am i right?" in message.content.lower():
        ...
        await message.channel.send("what colour is your jett rp bot role?")
    ...
    if any(bad_word in message.content.lower() for bad_word in bad_words):
        ...
        log = open("botlog.txt", "a",  encoding="utf8")
        log.write(f"{message.author} tried to send blacklisted messages | {time.time()} \n")
        log.close()
        database = sqlite3.connect("bot.db")
        c = database.cursor()
        queryresult2 = c.execute(f"SELECT warncount, reason FROM warns WHERE uid = \"{message.author.id}\"")
        queryresultfetched2 = queryresult2.fetchone()
        if queryresultfetched2 is not None:
            userwarns = queryresultfetched2[0]
            if userwarns+1 >= 3:
                modchannel = bot.get_channel(1208826268246876312)
                embed = discord.Embed(title = "Warn - Action", description = f"**{message.author} has earned 5 warns, last reason: {queryresultfetched2[1]} \nplease take an action bellow...**", color=discord.Color.blue())
                await modchannel.send(embed = embed, view=WarnAction(user=message.author, modchannel=modchannel))
                log = open("botlog.txt", "a",  encoding="utf8")
                log.write(f"{message.author} got muted due to warn thresholds | {current_time} \n")
                log.close()
            else:
                c.execute(f"UPDATE warns SET warncount = {userwarns+1} WHERE uid = \"{message.author.id}\"")
        else:
            c.execute(f"INSERT INTO warns (uid, username, warncount, date, reason, moderator, modname) VALUES (\"{message.author.id}\", \"{message.author}\", 1, \"{datetime.now()}\", \"Bad word usage\", \"System\", \"System\")")
        c.close()
        database.close()
